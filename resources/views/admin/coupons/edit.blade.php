<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="space-x-2">
                <a href="{{ route('admin.coupons.show', $coupon) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                    View Coupon
                </a>
                    ...
                    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Leave empty for unlimited usage. Current usage: {{ $coupon->usage_count }}
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">Uncheck to deactivate this coupon.</p>
                        </div>

                        <!-- Usage Information -->
                                                <div class="bg-green-50 p-4 rounded-lg">
                            <h3 class="text-lg font-medium text-green-900 mb-3">Usage Information</h3>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="font-medium text-green-800">Times Used:</span>
                                    <span class="text-green-600">{{ $coupon->usage_count }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="font-medium text-green-800">Usage Limit:</span>
                                    <span class="text-green-600">{{ $coupon->usage_limit ?? 'Unlimited' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="font-medium text-green-800">Created:</span>
                                    <span class="text-green-600">{{ $coupon->created_at->format('M d, Y') }}</span>
                            ...
                            <div class="bg-white p-4 rounded border-2 border-dashed border-gray-300">
                                <div class="text-center">
                                    ...
                                    <div class="text-lg text-gray-600 mt-1">
                                        <span id="preview-discount">{{ $coupon->discount_percentage }}</span>% OFF
                                    </div>
                                    <div class="text-sm text-gray-500 mt-2">
                                        <div id="preview-expiry">
                                            {{ $coupon->expiry_date ? 'Expires: ' . $coupon->expiry_date->format('M d, Y') : 'No expiry' }}
                                        </div>
                                        <div id="preview-limit">
                                            {{ $coupon->usage_limit ? 'Limited to ' . $coupon->usage_limit . ' uses' : 'Unlimited usage' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
