<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Payment for {{ $student->name }}'s Session</h1>
            <a href="{{ route('teacher.booking.calendar', ['student' => $student->id]) }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition duration-200">
                Back to Calendar
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <div class="flex items-center space-x-3">
                                    @if($student->studentProfile && $student->studentProfile->profile_image)
                                        <img src="{{ asset('storage/profile_images/' . $student->studentProfile->profile_image) }}" 
                                            alt="{{ $student->name }}" class="w-12 h-12 object-cover rounded-full">
                                    @else
                        ...
                        <div>
                            ...
                            <form method="POST" action="{{ route('teacher.booking.payment.process', ['student' => $student->id]) }}" id="payment-form">
                                @csrf
                                ...
                                <div class="mb-6">
                                    <p class="text-sm text-gray-600">
                                        By completing this payment, you agree to book this chess session for {{ $student->name }}. The payment method will be securely stored for future bookings.
                                    </p>
                                </div>
                                ...
                                <button type="submit" class="w-full bg-blue-600 text-white py-3 px-4 rounded-md hover:bg-blue-700 transition duration-200 font-medium" id="submit-button">
                                    <div class="flex items-center justify-center">
                                        ...
                                        <span id="button-text">Setup Payment & Book Session - £{{ number_format($price, 2) }}</span>
                                    </div>
                                </button>
                            </form>
                        </div>
    ...
    <script>
        ...
        form.addEventListener('submit', async function(event) {
            ...
            if (result.error) {
                ...
                buttonText.textContent = 'Setup Payment & Book Session - £{{ number_format($price, 2) }}';
            } else {
